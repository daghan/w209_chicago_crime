...
colnames(broken.alley.lights.by.ZIP) <- c("Date", "Zip","Broken.Lights")

# merge it with the per_day, per_zip crime data
merged_data = merge(x = sum.crimes.by.date.zip, y = broken.alley.lights.by.ZIP, by = c("Date","Zip"), all.y = T)
